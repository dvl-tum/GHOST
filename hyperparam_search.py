...
def train_model(config):
    ...
    model, input_size, params_to_update = get_model(args.model_name, num_classes, args.feature_extract, device)
    ...
    dataloaders = get_data_loaders(data_dir, args.oversampling, input_size, train_indices, config['batch_size'])
    ...
    for epoch in range(args.num_epochs):
        ...
        for phase in ['train', 'val']:
            epoch_loss, epoch_acc = rum_model(phase, model, dataloaders, optimizer, criterion, device)
...
def run_model(phase, model, dataloaders, optimizer, criterion, device):
...
def get_model(model_name, num_classes, feature_extract, device):
...
def get_data_loaders(data_dir, oversampling, input_size, train_indices, batch_size):
...
if __name__ == '__main__':
    ...
    search_space = {
        ...
        'weight_decay': tune.uniform(0.001, 0.0001),
        'batch_size': tune.grid_search([8, 32, 64])
    }

    train_model(config={'lr':0.001, 'momentum': 0.9, 'weight_decay': 0.0001, 'batch_size': 8})

    """analysis = tune.run(train_model, config=search_space)

    analysis = tune.run(train_model, name="exp", scheduler=sched,
