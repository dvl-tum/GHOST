...
from src.datasets.ProxyDataset import ProxyDataset
...
class ProxyManager(Manager):
    def __init__(self, device, timer, dataset_cfg, reid_net_cfg, 
        ...
        super(ProxyManager, self).__init__(device, timer, dataset_cfg, reid_net_cfg, tracker_cfg)
    ...
    def _get_loaders(self, dataset_cfg):
        ...
        for mode in _SPLITS[dataset_cfg['splits']].keys():
            ...
            if mode != 'train':
                ...
                dataset = ProxyDataset(dataset_cfg['splits'], seqs, 
                                        dataset_cfg, self.dir) 
                loaders[mode] = DataLoader(dataset, batch_size=1, shuffle=False,
    ...
    def _setup_training(self):
        ...
        param_groups = [{'params': list(set(self.proxy_gen.parameters())),
                            'lr': self.reid_net_cfg['train_params']['lr']}]
        ...
        self.loss2 = ReID.utils.losses.CrossEntropyLabelSmooth(
                        num_classes=self.num_classes, dev=self.device).to(self.device)
        '''
        self.loss3 = ReID.utils.losses.CrossEntropyLabelSmooth(
                        num_classes=self.num_classes, dev=self.device).to(self.device)
    ...
    def _check_best(self, mota_overall, idf1_overall):
        if mota_overall > self.best_mota:
                ...
                torch.save(self.proxy_gen.state_dict(),
                            osp.join(self.save_folder_nets,
                                    'proxy_gen_' + self.fn + '.pth'))

    def _save_best(self):
        os.rename(osp.join(self.save_folder_nets, 'proxy_gen_' +  self.fn + '.pth'),
            osp.join(self.save_folder_nets_final, 
                    str(self.best_mota) + 'proxy_gen_' +  
    ...
    def _train(self, e):
        ...
        for data in self.loaders['train']:
            ...
            if self.gnn:
                ...
                '''
            feats3 = list()
            hid = torch.zeros(self.proxy_gen.h_dim).to(self.device)
            for y, f in zip(Y, feats2[-1]):
                f3, hid = self.proxy_gen(f, hid)
                feats3.append(f3)

            feats3 = torch.stack(feats3).to(self.device)
            preds3 = self.gnn.fc(feats3)
            
            loss = self.loss3(preds3/temp, Y)
            losses['Loss 3'].append(loss)
