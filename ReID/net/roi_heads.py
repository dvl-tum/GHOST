...
class RoIHeads(nn.Module):
    ...
    def check_targets(self, targets):
        ...
        if "ids" in targets[0].keys():
            assert all(["ids" in t for t in targets])
        else:
    ...
    def select_training_samples(self,
        ...
        if "ids" in targets[0].keys():
            id_labels = [t["ids"] for t in targets]
        else:
            gt_labels = [t["labels"] for t in targets]

        if "ids" not in targets[0].keys():
    ...
    def postprocess_detections(self,
                               ...
                               box_regression=None,  # type: Tensor
                               proposals=None,       # type: List[Tensor]
                               image_shapes=None     # type: List[Tuple[int, int]]
                               ):
        ...
        for boxes, scores, image_shape in zip(pred_boxes_list, pred_scores_list, image_shapes):
            ...
            if box_regression: # Jenny: DO NOT DO THIS BECAUSE WE WANT TO HAVE ALL PROPOSALS
            ...
            # Jenny: I don't quite get this part here....
    ...
    def forward(self,
        ...
        if targets is not None:
            for t in targets:
                ...
                if "ids" in targets[0].keys():
                    assert t["public_preds"].dtype in floating_point_types, 'public proposal boxes must of float type'
                    assert t["ids"].dtype == torch.int64, 'id target labels must of int64 type'
                else:
        ...
        if self.training and "ids" not in targets[0].keys(): # TODO: maybe go not into here!!
            ...
            matched_idxs = None
            id_targets = [t["ids"] for t in targets]
        ...
        if self.training:
            ...
            if self.box_predictor.additional_embedding:
                boxes, scores, labels = self.postprocess_detections(class_logits, proposals=proposals, image_shapes=image_shapes)
            else:
                ...
                result.append(
                    {
                        ...
                        "id_targets": id_targets[i]
                    }
                )

        # NOT IMPORTANT for me (no masks)
        if self.has_mask():
        ...
        # compile each branch
        # NOT IMPORTANT for me (just mask)
