...
def bn_inception(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load('net/finetuned_cub.pth'))
        # model.load_state_dict(torch.load('net/cub_good/cub_gtg_yBFJH331QcC1m4At.pth'))

        # model.load_state_dict(torch.load('/usr/stud/elezi/PycharmProjects/gtg_embedding/stanford_radam_scheduler/stanford_gtg_R4fPNGF8Xi96TUNx.pth'))
        # model.load_state_dict(torch.load('net/cub_gtg_XWVa9Oy92IGmbnT6.pth'))
        # model.load_state_dict(torch.load('net/stanford_gtg_best.pth'))
        # model.load_state_dict(torch.load('net/finetuned_Stanford.pth'))
        # model.load_state_dict(torch.load('net/stanford_gtg_qLPM4aq0r27YbYF6.pth'))  # trained on 30 epochs

    return model

class BNInception(nn.Module):

    def __init__(self, nb_classes = 100, is_pretrained = True):
        # super(self).__init__()
