...
def restricted_softmax(src, index, dim, dim_size, margin: float = 0.):
    src_max = torch.clamp(scatter_max(src, index, dim=dim, dim_size=dim_size)[0], min=0.)
...
class AttentionLayerDot(MessagePassing):
    ...
    def __init__(self, channels: int, heads: int = 1, groups: int = 1,
                 dropout: float = 0., gcn_norm: bool = False,
    ...
    def forward(self, x: Tensor, edge_index: Tensor,
        ...
        if edge_weight.shape[-1] != 2:
            edge_weight = edge_weight.transpose(-2, -1)
